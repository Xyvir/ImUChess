...
function command(text, skipOpening) {
  ...
  if (text.split("/").length == 8 && text.split(".").length == 1) {
    ...
    text = text.replace(/\u2605/g, "");
    var headers = {};
    text = text.replace(/\[([A-Za-z0-9]+)\s+"([^"]+)"\]/g, function (m, key, value) {
      headers[key] = value;
      return "";
    });
    _pgnHeaders = headers;
    ...
    // Pre-calculate target game
    for (var i = 0; i < moves.length; i++) {
      if (moves[i].length == 0 || "*".indexOf(moves[i][0]) == 0) continue;
      if (moves[i].indexOf("FEN.") == 0) continue; // Skip FEN setups for now in guess mode or handle differently
      if (moves[i] == "--") {
        pos.w = !pos.w;
        _targetGame.push({ san: "--", from: null, to: null, p: null });
        continue;
      }
      var move = parseMove(pos, moves[i]);
      if (move != null) {
        _targetGame.push({ san: moves[i], from: move.from, to: move.to, p: move.p, fen: generateFEN(pos) });
...
function showStatus(text, answer) {
  ...
  if (answer != null && answer.length == 4) {
    var move = {
      from: { x: "abcdefgh".indexOf(answer[0]), y: "87654321".indexOf(answer[1]) },
...
function historySave() {
  var data = {
    _historyindex: _historyindex, _history: _history, _history2: _history2,
...
function onMouseUp(e) {
  ...
  if (!_dragActive && _clickFrom != null && _clickFromElem != null && _clickFromElem.className.indexOf(" h0") > 0) {
    ...
    if (legal) {
      ...
      showBoard(getCurFEN() == oldfen);
      _dragElement = null;
      if (_guessMode) checkGuess(move, oldfen); else doComputerMove();
...
function loadEngine() {
  ...
  engine.eval = function evalPos(fen, done, info) {
...
function parseBestMove(m) {
  ...
  return {
    from: { x: "abcdefgh".indexOf(m[0]), y: "87654321".indexOf(m[1]) },
...
function playNextTargetMove() {
  ...
  var pos = parseFEN(getCurFEN());
  var userColor = _flip ? false : true;
  ...
  setTimeout(function () {
    historyAdd(getCurFEN());
    setCurFEN(generateFEN(pos2));
    historyAdd(getCurFEN(), null, move, move.san);
    showBoard();
    _targetMoveIndex++;

    if (_targetMoveIndex >= _targetGame.length) {
      showStatus("Game Over! Good job.");
    } else {
      showStatus("Your Turn!");
    }
    _processingGuess = false;
...
function checkGuess(userMove, oldfen) {
  ...
  var matched = userMove.from.x == target.from.x && userMove.from.y == target.from.y &&
    userMove.to.x == target.to.x && userMove.to.y == target.to.y;
  ...
  if (matched) {
    ...
    var userFen = getCurFEN();

    var pos = parseFEN(oldfen);
    var posTarget = doMove(pos, target.from, target.to, target.p);
    var targetFen = generateFEN(posTarget);
    ...
    var checkDone = function () {
      if (userScore != null && targetScore != null) {
        ...
        var myUserScore = -userScore;
        var myTargetScore = -targetScore;
    ...
    var evalPosition = function (fen, callback) {
      _engine.depth = 15;
      _engine.eval(fen, function (str) {
        callback(_engine.score);
      });
    };

    evalPosition(userFen, function (score) {
      userScore = score;
      evalPosition(targetFen, function (score2) {
        targetScore = score2;
        checkDone();
      });
    });
